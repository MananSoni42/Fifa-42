...
class Menu:
    ...
    def create_sett_menu(self):
        ...
        s1.change = lambda col: self.color_change(s1, col, self.team1) # set team 1's color
        s1.set_background_color((255,165,0))

        s2 = sett_menu.add_selector('Team 2 color:', colors, default=6, font_color=(255,255,255))
        s2.change = lambda col: self.color_change(s2, col, self.team2) # Set team 2's color
        s2.set_background_color((0,0,255))

        sett_menu.add_selector('Sound', [('ON', True), ('OFF', False)], default=0, onchange=self.set_menu_sound)
    ...
    def create_form_menu(self):
        ...
        form_menu.add_selector('Team 1 formation:  ', form_opts, default=self.selected_formation[1][1], onchange=lambda name, id: self.set_form(id,team_id=1))
        form_menu.add_selector('Team 2 formation:  ', form_opts, default=self.selected_formation[2][1], onchange=lambda name, id: self.set_form(id,team_id=2))
    ...
    def color_change(self, widget, col, team): # Change color of widget and corresponding team
